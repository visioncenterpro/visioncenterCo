...
//$conn_access = odbc_connect("Driver={Microsoft Access Driver(*.mdb)};Dbq=Z:\PlantaServidorGeneral", "", "");
...
switch ($_GET['option']) {
    ...
    case "Pedidos":
        ...
        // Muelbes SD
        if ($IdSolicitud != 0) {
            $mueblesSD = "SELECT IdMuebleDesarrollo FROM SolicitudSubMueblesSD WHERE IdSolicitud = $IdSolicitud AND IdMuebleDesarrollo is not null";
            $resultc = odbc_exec($conn_access, $mueblesSD);
            while ($fila = odbc_fetch_array($resultc)) {
                $validacion_mueble = "SELECT * FROM access_forniture WHERE id_forniture = ".$fila['IdMuebleDesarrollo'];
                $result = $Mysqlivision->query($validacion_mueble);
                $validancion = 0;
                while ($vali = mysqli_fetch_array($result)) {
                    $validancion = 1;
                }
                if ($validancion == 0) {
                    ...
                    // muebles ME
                    $SqlItem = "SELECT Mueble.IdMueble, Mueble.CódigoCliente, Mueble.Versión, Mueble.DescripciónMueble, Mueble.DescripciónDesarrollo, Mueble.CódigoDestino, Mueble.Observaciones, Mueble.IdClienteME, Mueble.IdLinea, Mueble.IdLáminaDimensión, Mueble.Pinta FROM Mueble ORDER BY Mueble.IdMueble;";
                    $result = odbc_exec($conn_access, $SqlItem);
                    //print_r(odbc_error());

                    while ($fila2 = odbc_fetch_array($result)) {
                        $version = ($fila2['Versión'] == "") ? 0 : $fila2['Versión'];
                        $idcliente = ($fila2['IdClienteME'] == "") ? 0 : $fila2['IdClienteME'];
                        $linea = ($fila2['IdLinea'] == "") ? 0 : $fila2['IdLinea'];
                        $dimension = ($fila2['IdLáminaDimensión'] == "") ? 0 : $fila2['IdLáminaDimensión'];
                        ...
                        $Mysqlivision->query("INSERT INTO access_forniture (id_forniture,type_forniture,code_client,`version`,description,development,destination,observation,id_client,`line`,
                                sheet_format, color) VALUES (" . $fila2['IdMueble'] . ", 'ME','" . $fila2['CódigoCliente'] . "', " . $version . ", '" . $fila2['DescripciónMueble'] . "', '" . $fila2['DescripciónDesarrollo'] . "', '" . $fila2['CódigoDestino'] . "', '" . $fila2['Observaciones'] . "', " . $idcliente . ", " . $linea . ", " . $dimension . ",'" . $fila2['Pinta'] . "');");
                    }
                    // END muebles ME
                }else{
                    //echo "ya existe el mueble";
                }
        ...
        // ,IIf(CódigoDestino='30309012','SD','ME') as line
        $SqlSolicitudes = "SELECT Clientes.Cliente, Solicitudes.* FROM Solicitudes INNER JOIN Clientes ON Solicitudes.IdCliente = Clientes.IdCliente WHERE (((Solicitudes.IdSolicitud) = ".$IdSolicitud." )) ORDER BY IdSolicitud";
        //print_r($SqlSolicitudes);
        $result = odbc_exec($conn_access, $SqlSolicitudes);
        //print_r(odbc_num_rows($result));

        while ($fila = odbc_fetch_array($result)) {
            print_r($fila[utf8_decode('CódigoDestino')]);
            //print_r(mb_convert_encoding($fila['CódigoDestino'], "UTF-8"));
            $date = new DateTime($fila['FechaSolicitud']);
            $empaque = empty($fila['IdFormaEmpaque']) ? 0 : $fila['IdFormaEmpaque'];
            $dimension = ($fila[utf8_decode('IdLáminaDimensión')] == "") ? 0 : $fila[utf8_decode('IdLáminaDimensión')];
            $idcliente = ($fila['IdCliente'] == "") ? 0 : $fila['IdCliente'];
            $cliente = ($fila['Cliente'] == "") ? 0 : $fila['Cliente'];

            if ($fila[utf8_decode('CódigoDestino')] == '30309012') {
                $line = "SD";
            }else{
                $line = "ME";
            }
            ...
            $execute = $Mysqlivision->query("INSERT INTO access_order (`order`,`type`,`date`,id_client,client,project,destination,sheet_format,type_of_packaging,reference,line) "
                    ...
                    . " " . $idcliente . ",'".$cliente."','" . $fila['Proyecto'] . "','" . $fila[utf8_decode('CódigoDestino')] . "','" . $dimension . "'"
                    . "," . $empaque . ",'".$fila['ObservacionesMuebles']."','".$line."') ");
    ...
    case "Muebles":
        ...
        $mueblesSD = "SELECT IdMuebleDesarrollo FROM SolicitudSubMueblesSD WHERE ";
