<div class="col-md-7 table-wrapper-scroll-y">
    <table id="table_pack" class="table table-bordered table-striped table-condensed ">
        <thead>
            <tr>
                <th style="width:25px"><button class="btn btn-block btn-all btn-xs" onclick="AddAll()"><span class="fa fa-sign-in" aria-hidden="true"></span> All</button></th>
        ...
        <tbody>
            ...
            foreach ($headers as $t) :
                ...
                <tr class="test" pack="<?= $t->id_order_package_supplies ?>" undpack="<?= ($t->type_package == 1)?$t->quantity_per_package:$t->quantity_supplies ?>">
                    <?php if($delivered == 1) { ?>
                    <td ><button  class="btn btn-block btn-primary btn-xs" disabled="disabled" onclick="AddItem('<?=$t->id_order_package_supplies?>','<?=$order?>')"><span class="fa fa-sign-in" aria-hidden="true"></span></button></td>
                    <?php }else{ ?>
                        <td ><button  class="btn btn-block btn-primary btn-xs" onclick="AddItem('<?=$t->id_order_package_supplies?>','<?=$order?>')"><span class="fa fa-sign-in" aria-hidden="true"></span></button></td>
                    <?php } ?>
                    ...
                    <?php } ?>
                    <td style="text-align:center"><button class="btn btn-primary btn-sm" onclick="modal_detail('<?=$t->id_order_package_supplies?>','<?=$t->number_pack?>','<?=$t->order?>')"><span class="fa fa-eye" aria-hidden="true"></span></button></td>
                </tr>
...
<div class="col-md-5 table-wrapper-scroll-y">
    <table id="table_detail" class="table table-bordered table-striped table-condensed ">
        <thead>
            <tr>
                <th style="width:25px"><button class="btn btn-block btn-danger btn-xs" onclick="Delete_all('<?=$order?>')"><span class="fa fa-trash-o" aria-hidden="true"></span> All</button></th>
        ...
        <tbody>
           ...
           foreach ($detail as $t) : ?>
                <tr id="tr-<?=$t->id_order_package_supplies?>">
                    <td ><button class="btn btn-block btn-danger btn-xs" onclick="DeleteDetail('<?= $t->id_delivery_supplies_detail?>','<?= $t->id_order_package_supplies?>','<?= $order?>')"><span class="fa  fa-trash-o" aria-hidden="true"></span></button></td>
                    <td style="text-align:center"><?= $t->number_pack ?></td>
                    <td style="text-align:center"><?= $t->quantity_pq ?></td>
                    <td style="text-align:center"><?= $t->weight_per_package ?></td>
