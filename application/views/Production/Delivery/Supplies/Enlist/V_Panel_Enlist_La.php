...
<script>
    $(function () {
        ...
        $("#btnload").click(function () {
            if (ValidateInput("txt-order")) {
                ...
                $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/SearchOrder", {order: order}, function (data) {
                    if (data.res == "OK") {
                        if (data.rows > 0) {
    ...
    function search(order){
        ...
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/SearchOrder2", {order: order}, function (data) {
            if (data.res == "OK") {
                if (data.rows > 0) {
                    $("#content-table").html(data.table);
                    TableData("table_supplies", false, false, true);
    ...
    function Detail_replaced(order,id_order_supplies){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Detail_replaced",{order:order,id_order_supplies:id_order_supplies},function(data){
            console.log(data);
    ...
    function modal_synchronize(order){
        ...
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/data_synchronize",{order:order},function(data){
            console.log(data);
    ...
    function synchronize(){
        ...
        var date = $("#date").val();
        var date2 = $("#date2").val();
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/synchronize_items_ax",{order:order,date:date,date2:date2},function(data){
            console.log(data);
            $(".overlay_ajax").hide();
            $(".loader_ajax2").text("");
    ...
    function modal_new_item(order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/data_new_item",{order:order},function(data){
    ...
    function save_new_item(){
        ...
        if(code == "" || name == "" || cnt == "" || weight_unt == ""){
            ...
            $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/save_new_item",{order:order, code:code, name:name, unity:unity, type:type, cnt:cnt, weight_unt:weight_unt, observation:observation},function(data){
    ...
    function Add_to_order(order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/get_data_add_to_order",{order:order},function(data){
            $("#order").text(order);
    ...
    function Add_new_to_order(){
        ...
        if(quantity == ""){
            ...
            $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Add_new_to_order",{order:order,id_supplies:id_supplies,quantity:quantity,observation:observation},function(data){
        ...
        if(cnt != ""){
            if(id_supplies == ""){
                ...
                swal({
                    ...
                    if(result){
                        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Replace_to_order",{order:order,id_supplies:id_supplies,supplies:supplies,cnt:cnt,observation:observation},function(data){
                            if(data.res == "error_vali"){
                                swal({title: 'Atención', text: 'El Item seleccionado no se encuentra en AX, por favor actualize los datos', type: 'error'});
                            }else{
                                if(data.vali == "error"){
    ...
    function Delete_to_order(order,id_order_supplies){ // cambio de estado
        swal({
            ...
            if (result) {
                $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Delete_to_order",{order:order,id_order_supplies:id_order_supplies},function(data){
                    search(order);
                    $("#modal_to_order").modal("hide");
                    swal({title: '', text: '', type: 'success'});
                },'json').fail(function (error) {
    ...
    function Pending(order){
        var win = window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/Pending_Report_Supplies/"+order+"", '_blank');
        win.focus();
    }
    
    function Manual_Package(order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/get_data_manual",{order:order},function(data){
            console.log(data);
            $("#order").text(order);
    ...
    function modal_edit(id_order_package_supplies,number_pack,order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/get_data_edit",{id_order_package_supplies:id_order_package_supplies,number_pack:number_pack,order:order,type:'A'},function(data){
    ...
    function modal_add_supplies(id_order_package_supplies,number_pack,order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/get_data_add",{order:order,number_pack:number_pack},function(data){
    ...
    function delete_pq(order,number_pack){
        ...
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/validation_supplies",{order:order,number_pack:number_pack},function(data){
            console.log(data);
            $("#content-table").html(data.table);
            TableData("table_supplies", false, false, true);
            TableData("table_pack", false, false, true);
    ...
    function Add_packed(id_supplies,pack){
        ...
        if(weight_package == ""){
            ...
            $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Add_Packed",{id_supplies:id_supplies,order:order,pack:pack,package_number:package_number,pack_total:pack_total,weight_package:weight_package,count:count,array_order_supplies:array_order_supplies,array_id_supplies:array_id_supplies,array_check:array_check},function(data){
                $(".overlay_ajax").hide();
                $(".loader_ajax2").text("");
    ...
    function validation_header_re(order,pack_number){
        $(".overlay_ajax").show();
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/validation_header_RE",{order:order,number_pack:pack_number},function(data){
            console.log(data);
            $("#content-table").html(data.table);
            TableData("table_supplies", false, false, true);
            TableData("table_pack", false, false, true);
    ...
    function go_back_all(){
        ...
        array.forEach(function(element){
            ...
            console.log(pack);
            if(pack != "0"){
                $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Go_Back_Pack",{id_supplies:id_supplies,order:order,id_order_supplies:id_order_supplies,package_number:package_number,count:count},function(data){
                    console.log(data);
                    //if(count == total){
                        $("#content-table").html(data.table);
                        TableData("table_supplies", false, false, true);
    ...
    function go_back(id_supplies,order,id_order_supplies,position){
        var package_number = $("#package_number").val();
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/Go_Back_Pack",{id_supplies:id_supplies,order:order,id_order_supplies:id_order_supplies,package_number:package_number},function(data){
            console.log(data);
            $("#content-table").html(data.table);
            TableData("table_supplies", false, false, true);
            TableData("table_pack", false, false, true);
    ...
    function validation_header(order){
        ...
        //--*************************--//
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/validation_header",{number_pack:number_pack,order:order,weight_package:weight_package,count:count},function(data){
            console.log(data);
        },'json').fail(function (error) {
            swal({title: 'Error Toma un screem y envialo a sistemas!', text: error.responseText, type: 'error'});
        });
    ...
    function deleted(order){
        var win = window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/Report_Deleted/"+order+"", '_blank');
        win.focus();
    }
    
    function PDF(order){
        var win = window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/PDF_supplies/"+order+"", '_blank');
        win.focus();
    }
    ...
    function UpdateSupplies(){
        if(validatefield()){
            ...
            $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/UpdateWeightSupplies",{GeneralArray:GeneralArray},function(data){
                if(data.res == "OK"){
    ...
    function Enlist(order){
        if(parseInt($("#count").text()) > 0 ){
            window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/Print_Enlist/"+order, '_blank');
        }else{
    ...
    function Tags(order){
        if(parseInt($("#count").text()) > 0 ){
            window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/GeneratePacksSDSupplies/"+order, '_blank');
    ...
    function Exclude(){
        ...
        var obs = $("#obs").val();
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/UpdateOrderSupplies", {id_order_supplies: id_order_supplies,field:'exclude', value: exclude, obs:obs}, function (data) {
    ...
    function GeneratePacks(order){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/GeneratePacks", {order: order}, function (data) {   
            if(data.res == "OK"){
