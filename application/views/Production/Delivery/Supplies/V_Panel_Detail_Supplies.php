...
<script>
    ...
    $(function () {
        $("#btn-print").click(function () {
            window.open("<?= base_url() ?>Production/Delivery/C_Delivery/Print_Deliver_Supplies/<?= $order ?>/<?= $delivery ?>", '_blank');
        });
    ...
    function SplitPack(id_order_package_supplies) {
        if (ValidateInput("quantity-split")) {
            if ($("#quantity-split").val() > 0 && parseFloat($("#quantity-split").val()) < parseFloat($("#t-undpack").val())) {
                $.post("<?= base_url() ?>Production/Delivery/C_Delivery/SplitPackSupplies", {id_order_package_supplies: id_order_package_supplies, quantity: $("#quantity-split").val(), delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
                    if (data.res == "OK") {
    ...
    function AddAll() {
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery/AddAllSuppliesToDelivery", {delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
            if (data.res == "OK") {
                $("#tables").html(data.tables);
            }
        }, 'json');
    ...
    function modal_detail(id_order_package_supplies,number_pack,order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery/get_data_edit",{id_order_package_supplies:id_order_package_supplies,number_pack:number_pack,order:order,type:'E'},function(data){
    ...
    function Approve(status) {
        ...
        swal({
            ...
            if (result) {
                $.post("<?= base_url() ?>Production/Delivery/C_Delivery/ApproveDeliverSupplies", {delivery:<?= $delivery ?>, order:<?= $order ?>, status: status, observation: $("#observation").val()}, function (data) {
                    if (data.res == "OK") {
                        disabledInput();
    ...
    function Deliver() {
        if ($("#table_detail > tbody >tr").length > 0) {
            swal({
                ...
                if (result) {
                    $.post("<?= base_url() ?>Production/Delivery/C_Delivery/DeliverSupplies", {delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
                        if (data.res == "OK") {
                            disabledInput();
    ...
    function Delete_all(order){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery/DeleteSuppliesDeliveryAll", {delivery:<?= $delivery ?>,order:order}, function (data) {
            if (data.res == "OK") {
                $("#tables").html(data.tables);
            }
        }, 'json');
    ...
    function DeleteDetail(id_delivery_supplies_detail, id_order_package_supplies,order) {
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery/DeleteSuppliesDelivery", {delivery:<?= $delivery ?>,id_delivery_supplies_detail: id_delivery_supplies_detail, id_order_package_supplies: id_order_package_supplies, order:order}, function (data) {
            if (data.res == "OK") {
    ...
    function AddItem(id_order_package_supplies, order) {
        if ($("#tr-" + id_order_package_supplies).length <= 0) {
            ...
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery/AddPackToDeliverySupplies", {delivery:<?= $delivery ?>,id_order_package_supplies: id_order_package_supplies, delivery:<?= $delivery ?>, quantity: 1, order:order}, function (data) {
                if (data.res == "OK") {
                    $("#tables").html(data.tables);
                } else {
                    swal({title: 'Error!', text: data.res, type: 'error'});
    ...
    function UpdateDetailDelivery(id_delivery_supplies_detail, id_order_package_supplies, input) {
        if ($("#quantity-" + id_order_package_supplies).val() == 0 || $("#quantity-" + id_order_package_supplies).val() == "") {
            ...
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery/UpdateDetailDeliverySupplies", {id_delivery_supplies_detail: id_delivery_supplies_detail, id_order_package_supplies: id_order_package_supplies, quantity: $(input).val()}, function (data) {
                if (data.res == "OK") {
