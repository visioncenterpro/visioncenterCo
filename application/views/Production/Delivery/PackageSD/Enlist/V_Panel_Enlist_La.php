...
<script>
    $(function () {
        ...
        $("#btnload").click(function () {
            if (ValidateInput("txt-order")) {
                ...
                $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/SearchOrderPackSD", {order: order}, function (data) {
                    if (data.res == "OK") {
                        if (data.rows > 0) {
    ...
    function excel(order){
        $.post("<?= base_url()?>Production/Delivery/C_Delivery_La/excel_one_m",{order:order},function(data){
            var a=$("<a>");
    ...
    function Pending(order){
        var win = window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/Pending_Report/"+order+"", '_blank');
        win.focus();
    }
    
    function Enlist(order){
        if(parseInt($("#count").text()) > 0 ){
             window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/Generate_Resumen/"+order, '_blank');
        }else{
    ...
    function Savepiece(id_order_package){
        if(ValidateInput("peso")){
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/AddPieceToPack", {largo:$("#largo").val(),ancho:$("#ancho").val(),calibre:$("#calibre").val(),peso:$("#peso").val(), order:$("#txt-order").val(),id_order_package:id_order_package}, function (data) {
                if (data.res == "OK") {
    ...
    function DeletePiece(id_order_package_addpiece,id_order_package){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/DeletePieceToPack", {id_order_package_addpiece:id_order_package_addpiece,order:$("#txt-order").val(),id_order_package:id_order_package}, function (data) {
            if (data.res == "OK") {
    ...
    function OpenDeletePiece(id_order_package){
        if(parseInt($("#pz-add-"+id_order_package).text()) > 0){
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/CharguePieceToPack", {order:$("#txt-order").val(),id_order_package:id_order_package}, function (data) {
                    var rows = '';
    ...
    function Range(order) {
        if($("#tabla >tbody >tr").length <= 0){
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/GenerateRangeTag", {order:order}, function (data) {
                if(data.res == "OK"){
    ...
    function Range2(order) {
        if($("#tabla2 >tbody >tr").length <= 0){
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/GenerateRangeTag2", {order:order}, function (data) {
                if(data.res == "OK"){
    ...
    function Tags(order,min,max,btn){
        ...
        window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/PrintPack/" + order+"/"+min+"/"+max, '_blank');
    }
    function Tags2(order,min,max,btn){
        $(btn).addClass("btn-primary").removeClass("btn-default");
        window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/GeneratePacksSD2/" + order+"/"+min+"/"+max, '_blank');
    }
...
//
    function Weight(order){
        ...
        $(".overlay_ajax").show();
        if(parseInt($("#count").text()) > 0 ){
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/CalculateWeightPackSD", {order:order}, function (data) {
