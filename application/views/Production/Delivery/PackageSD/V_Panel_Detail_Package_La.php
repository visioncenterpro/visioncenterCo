...
<script>
    ...
    $(function(){
        ...
        $("#btn-print").click(function(){
            window.open("<?= base_url() ?>Production/Delivery/C_Delivery_La/Print_Deliver_PacksSD/<?= $order ?>/<?= $delivery ?>", '_blank');
        });
    ...
    function OpenReversePack(delivery,id_order_package,id_delivery_package_detail,quantity){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/ListRequestReverse", {id_order_package:id_order_package, id_delivery_package_detail:id_delivery_package_detail}, function (data) {
    ...
    function ReversePack(delivery,id_order_package,id_delivery_package_detail){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/ReversePack", {delivery:<?= $delivery ?>, order:<?= $order ?>,request:delivery,quantity:$("#rev-cantidad").val(),id_order_package:id_order_package, id_delivery_package_detail:id_delivery_package_detail}, function (data) {
            if (data.rs.res == "OK") {
    ...
    function AddAll() {
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/AddAllPackToDelivery", {delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
            if (data.res == "OK") {
                $("#tables").html(data .tables);
            }
        }, 'json');
    ...
    function RemoveAll() {
        if($("#table_detail > tbody > tr").length > 0){
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/RemoveAllPackToDelivery", {delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
                if (data.res == "OK") {
                    $("#tables").html(data.tables);
                }
            }, 'json');
    ...
    function add_furniture(){
        ...
        all.forEach(function(element) {
            if(element.id == furniture){
                ...
                if(sum > 0){
                    $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/Add_furniture", {delivery:<?= $delivery ?>, order:<?= $order ?>, furniture:furniture, pack:pack, sum:sum}, function (data) {
                        var e = data;
    ...
    function add_furniture_table(e){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/Add_furniture_table", {delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
            if (e == "OK") {
                $("#tables").html(data.tables);
            }
        }, 'json');
    ...
    function update_data_furniture(furniture){
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/update_data_furniture", {order:<?= $order ?>,furniture:furniture}, function (data) {
            console.log(data);
        }, 'json');
    }
    ...
    function Approve(status) {
        ...
        swal({
            ...
            if (result) {
                $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/ApproveDeliverPack", {delivery:<?= $delivery ?>, order:<?= $order ?>, status: status, observation: $("#observation").val()}, function (data) {
                    if (data.res == "OK") {
                        disabledInput();
    ...
    function Deliver() {
        if($("#table_detail > tbody >tr").length > 0){
            swal({
                ...
                if (result) {
                    $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/DeliverPacksSD", {delivery:<?= $delivery ?>, order:<?= $order ?>}, function (data) {
                        if (data.res == "OK") {
                            disabledInput();
    ...
    function DeleteDetail(id_delivery_package_detail, id_order_package, id_delivery) {
        var order = $("#order").val();
        $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/DeletePackToDelivery", {id_delivery_package_detail: id_delivery_package_detail, id_order_package: id_order_package,order:order,id_delivery:id_delivery}, function (data) {
            if (data.rs.res == "OK") {
    ...
    function AddItem(id_order_package, item, forniture, no, pack, id_furniture) {
        if ($("#tr-" + id_order_package).length <= 0) {
            ...
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/AddPackToDelivery", {id_order_package: id_order_package, delivery:<?= $delivery ?>, quantity:quantity ,order:order}, function (data) {
                if (data.rs.res == "OK") {
                    $("#delivered_quantity_" + id_order_package).text(data.rs.delivered_quantity);
                    $("#balance_" + id_order_package).text(data.rs.balance);
                    if (data.rs.balance <= 0) {
    ...
    function AddItems2(id_order_package, item, forniture, no, pack, quantity, id_delivery) {
        if ($("#tr-" + id_order_package).length <= 0) {
            var order = $("#order").val();
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/AddPackToDelivery2", {id_order_package: id_order_package, delivery:<?= $delivery ?>, quantity: quantity, id_delivery:id_delivery,order:order}, function (data) {
                if (data.rs.res == "OK") {
    ...
    function UpdateDetailDelivery(id_delivery_package_detail, id_order_package, input) {
        if($("#quantity-"+id_order_package).val() == 0 || $("#quantity-"+id_order_package).val() == ""){
            ...
            $.post("<?= base_url() ?>Production/Delivery/C_Delivery_La/UpdateDetailDelivery", {id_delivery_package_detail: id_delivery_package_detail, id_order_package: id_order_package, quantity: $(input).val()}, function (data) {
                if (data.res == "OK") {
