...
<script>
    ...
    $(function () {
        ...
        $(".return").click(function () {
            location.href = "<?= base_url() ?>Imos/Order/C_Order";
        });
    ...
    function ShowDetailsItem(id, order, cpid, idProadmin, nameid, med, pos) {
        location.href = "<?= base_url() ?>Imos/Order/C_Order/ShowDetailsItem/" + id + "/" + order + "/" + cpid + "/" + idProadmin + "/" + nameid+ "/" + med+ "/" + pos;
    }

    function ShowDetailsItemLocal(id, order) {
        location.href = "<?= base_url() ?>Imos/Order/C_Order/ShowDetailsItemLocal/" + id + "/" + order;
    }
...
//            $(".forniture > .form-group > .form-control").removeClass("required");
    ...
    function sendData(){
        ...
        if(array_body.length > 0){
            $.post("<?= base_url() ?>Imos/Order/C_Order/ValidateItemImport", {array_body: array_body}, function (data) {
                if (data.res == "OK") {
