...
<script>
    ...
    function sync_sheet_data(){
        if($("#date").val() == "" || $("#date2").val() == ""){
            ...
            $.post("<?= base_url()?>Imos/Sheets/C_Sheets/sync_sheet_data",{date:$("#date").val(),date2:$("#date2").val()},function(data){
                if(data.res == "OK"){
    ...
    function save(){
        ...
        if(code == "" || format == "" || caliber == "" || waste == ""){
            ...
            $.post("<?= base_url()?>Imos/Sheets/C_Sheets/save_sheet",{code:code,id_format:id_format,format:format,caliber:caliber,description:description,waste:waste},function(data){
                if(data.rs == "RE"){
    ...
    function update(){
        ...
        if(code == "" || format == "" || caliber == "" || description == "" || waste == ""){
            ...
            $.post("<?= base_url()?>Imos/Sheets/C_Sheets/update_sheet",{id:id,code:code,id_format:id_format,format:format,caliber:caliber,description:description,waste:waste},function(data){
                if(data.rs == "RE"){
                    swal({title: 'Atención', text: "El código de lámina ya existe", type: 'error'});
                }else{
                    $("#content").html(data.table);
