...
<script type="text/javascript">
    ...
    function AddRemission(id_request_cargo,id_remission,id_request_sd){
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/create_request_cargo", {id_request_cargo:id_request_cargo,id_remission:id_remission,id_request_sd:id_request_sd}, function (data) {
    ...
    function AddAll(id_request_cargo){
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/create_request_cargo_all", {id_request_cargo:id_request_cargo}, function (data) {
            swal({title: '', text: '', type: 'success'});
            data_view(id_request_cargo);
        }, 'json').fail(function (error) {
            swal({title: 'Error Toma un screem y envialo a sistemas!', text: error.responseText, type: 'error'});
    ...
    function delete_remission(id_request_cargue_detail,id_remission,id_request_sd){
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/delete_request_cargo", {id_request_cargue_detail:id_request_cargue_detail,id_remission:id_remission,id_request_sd:id_request_sd}, function (data) {
            swal({title: '', text: '', type: 'success'});
            data_view(<?=$id_request_cargo?>);
        }, 'json').fail(function (error) {
            swal({title: 'Error Toma un screem y envialo a sistemas!', text: error.responseText, type: 'error'});
    ...
    function DeleteAll(id_request_cargo){
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/delete_request_cargo_all", {id_request_cargo:id_request_cargo}, function (data) {
            swal({title: '', text: '', type: 'success'});
            data_view(id_request_cargo);
        }, 'json').fail(function (error) {
            swal({title: 'Error Toma un screem y envialo a sistemas!', text: error.responseText, type: 'error'});
    ...
    function data_view(id_request_cargo){
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/data_table_cargo_detail", {id_request_cargo:id_request_cargo}, function (data) {
    ...
    function generate_report(id_request_cargo){
        if(($("#driver").val() == 0 && $("#driver").val() == "") || ($("#license_plate").val() == "" || $("#license_plate").val() == "Pendiente")){
            ...
            $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/validate_request_cargo", {id_request_cargo:id_request_cargo,driver:driver,license_plate:license_plate,observation:observation,type_vehicle:type_vehicle}, function (data) {
                if (data.length > 0) {
                    var a = document.createElement('a');
                    a.href = '<?= base_url() ?>Dispatch/C_Dispatch_La/request_cargo/<?=$id_request_cargo?>';
    ...
    function license_plate(){
        ...
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/data_driver", {id_request_sd:id_request_sd}, function (data) {
    ...
    function pdf_requisition(id_remission,id_request_sd){
        var a = document.createElement('a');
        a.href = "<?= base_url() ?>Dispatch/C_Dispatch_La/PdfRequisition/"+id_remission+"/"+id_request_sd;
        a.setAttribute('target', '_blank');
        a.click();
    }

    function back(){
        window.location = "<?= base_url() ?>Dispatch/C_Dispatch_La/request_cargo_form";
    }
