...
<script>
    ...
    function CreateRequestSD() {
        swal({
            ...
            if (result) {
                var order = $("#order").val();
                $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/CreateRequestSD", {}, function (data) {
                    if (data.res == "OK") {
                        window.location.href = "<?= base_url() ?>Dispatch/C_Dispatch_La/InfoRequestDispatchSD/" + data.id;
                    } else {
                        if(data.res == "0"){
    ...
    function info(id_request){
        $.post("<?= base_url() ?>Dispatch/C_Dispatch_La/get_data_remission", {id_request:id_request}, function (data) {
