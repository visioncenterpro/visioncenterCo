        ...
        <tbody>
            <?php foreach ($content as $t) : ?>
                <tr id="tr-cont-<?= $t->id_order_package ?>">
                    ...
                    <td style="text-align:center;width:15px" ><button type="button" class="btn btn-danger btn-xs btn-tabla" onclick="Delete(<?= $t->id_request_detail ?>,<?= $t->id_order_package ?>,'Modulado',<?= $t->order ?>)"><i class="fa fa-trash"></i></button></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
