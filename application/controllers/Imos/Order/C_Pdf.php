...
class C_Pdf extends Controller {
    ...
    function CutList($order){
        ...
        //     $d['id'] = $i->ID;
        //     $d['cpid'] = $i->CPID;
            $d['tbody'][] = $this->CreateBodyPiecesRECut($i->ID, $order, $nameid, $i->POSSTR);
    ...
    function CreateBodyPiecesRECut($id, $order, $nameid, $pos){
        $PiecesRecord = $this->M_Order->ListPiecesALL($id, $order);
...
//            echo '<br><br>';
    ...
    function IronWorksAll2($order) {
        ...
        foreach ($items as $t) :
            ...
            if (!empty($t->INFO1)) {
                $nameid = $t->INFO1;
            }elseif (!empty($t->INFO2)) {
                $nameid = $t->INFO2;
            }elseif (!empty($t->INFO3)) {
                $nameid = $t->INFO3;
            }elseif ($t->CPID == 'PN') {
                $nameid = $t->CPID . $t->DEPTH;
            }else{
                $nameid = $t->CPID;
            }
...
//                    $und = (empty($descAX)) ? '' : $descAX->UNITID;
    ...
    function ExportLMAT(){
        ...
        $this->excel->getActiveSheet()->setTitle('Reporte LMAT');
        ...
        $this->excel->getActiveSheet()->setCellValue('I1', '80872399');
        $this->excel->getActiveSheet()->setCellValue('J1', '80872399');
        ...
        $res = 12 - strlen($order);
            ...
            $this->excel->getActiveSheet()->setCellValue('T1', "LM".$zero.$order."_14660");
        ...
        header('Content-Disposition: attachment;filename="'.$order.'.csv"; charset=UTF-8');
