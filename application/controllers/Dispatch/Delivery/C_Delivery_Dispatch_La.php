...
class C_Delivery_Dispatch_La extends Controller {
    ...
    public function index() {
        ...
        $data['all'] = $all['num'];
        $this->load->view('Dispatch/Delivery/V_Search_La',$data);
    ...
    function ListarDelivery($id_delivery,$type){
        ...
        foreach ($rows['result'] as $v) {
            $array[] = array($v->type, $v->id, $v->date, $v->order, $v->client, '<span class="label label-' . $v->color . '">' . $v->description . '</span>', '<button class="btn btn-block btn-primary btn-xs" onclick="window.location.href = \' '.base_url().'Production/Delivery/C_Delivery_La/'.$v->view.'/'.$v->id.'/'.$v->order.'/Dispatch \'"><span class="fa fa-sign-in" aria-hidden="true"></span></button>');
        }
