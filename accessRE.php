...
//$Mysqlivision = new mysqli('192.168.5.30', 'sistemas', 'sistemas', 'vision');
$Mysqlivision = new mysqli('192.168.5.209', 'developer', '123456789', 'vision');
if (!$Mysqlivision) {
    $res =  "Error: No se pudo conectar a MySQL." . PHP_EOL;
    $res .=  "errno de depuraci�n: " . mysqli_connect_errno() . PHP_EOL;
    CreateJson($res);
    exit;
}
...
switch ($_GET['option']) {
    ...
    case "Pedidos":
        ...
        $SqlSolicitudes = "SELECT Clientes.Cliente, Solicitudes.*,IIf([C�digoDestino]='30309012','SD','ME') as line
        ...
        while ($fila = odbc_fetch_array($result)) {
            ...
            $dimension = ($fila['IdL�minaDimensi�n'] == "") ? 0 : $fila['IdL�minaDimensi�n'];
            ...
            $execute = $Mysqlivision->query("INSERT INTO access_order (`order`,`type`,`date`,id_client,client,project,destination,sheet_format,type_of_packaging,reference,line) "
                    ...
                    . " " . $idcliente . ",'".$cliente."','" . $fila['Proyecto'] . "','" . $fila['C�digoDestino'] . "','" . $dimension . "'"
        ...
        $SqlItem = "SELECT Solicitudes.Solicitud, SolicitudSubMuebles.IdMueble, SolicitudSubMuebles.CantidadMueblesSolicitada, SolicitudSubMuebles.IdL�minaDimensi�n, SolicitudSubMuebles.Especificaciones
        ...
        while ($fila = odbc_fetch_array($result)) {
            ...
            $mueblesSD = "SELECT IdMuebleDesarrollo FROM MueblesSD where IdSolicitudMuebleSD = ".$fila['IdMueble'];
            ...
            while ($fila = odbc_fetch_array($result)) {
                ...
                $Mysqlivision->query("INSERT INTO access_forniture (id_forniture,id_sipcom,type_forniture,item,description,development,destination,observation,`line`,
                sheet_format, color) VALUES (" . $fila['IdMueble'] . "," . $row[0] . ", 'SD'," . $row[11] . ",'" . $row[14] . "', '" . $row[14] . "', '30309012','" . $row[9] . "', '5'," . $row[30] . ",'" . $row[39] . "');");
        ...
        while ($fila = odbc_fetch_array($result)) {
            ...
            $piezasAccess = "SELECT SolicitudSDSubPiezas.IdSolicitudMuebleSD, select_MsLaminas_AX.idInsumoERP, SolicitudSDSubPiezas.Pieza, 
                SolicitudSDSubPiezas.CantidadPiezas, SolicitudSDSubPiezas.C�digoL�minaInterno, SolicitudSDSubPiezas.LargoPieza, SolicitudSDSubPiezas.AnchoPieza, 
                ...
                SolicitudSDSubPiezas.EsNuevaPieza, SolicitudSDSubPiezas.CodArticuloAX, MSL�minasInterno.C�digoCalibre
            FROM (SolicitudSDSubPiezas LEFT JOIN select_MsLaminas_AX ON SolicitudSDSubPiezas.C�digoL�minaInterno = select_MsLaminas_AX.C�digoL�minaInterno) LEFT JOIN MSL�minasInterno ON SolicitudSDSubPiezas.C�digoL�minaInterno = MSL�minasInterno.C�digoL�minaInterno
            ...
            while ($row = odbc_fetch_array($resultPz)) {
               ...
               $execute = $Mysqlivision->query("INSERT INTO `access_order_pieces` (`id_order_item`, `piece`, `quantity`,caliber,code_sheet,code_sheet_ax, `long`, `width`,longF,widthF,weight, `code_canto_l1`, `code_canto_l2`, 
                    ...
                    `observation`,delicate,`print`,assembled_door) VALUES ($id_access_order_item, '".$row['Pieza']."', ".$qtyP.",".$cal.",'".$row['C�digoL�minaInterno']."','".$row['idInsumoERP']."', ".$longP.", ".$widthP.",".$longF.",".$widthF.",".round($w,6).",'".$l1."', '".$l2."', '".$a1."', '".$a2."', 
    ...
    case "Muebles":
        ...
        // muebles ME
        $SqlItem = "SELECT Mueble.IdMueble, Mueble.C�digoCliente, Mueble.Versi�n, Mueble.Descripci�nMueble, Mueble.Descripci�nDesarrollo, Mueble.C�digoDestino, Mueble.Observaciones, Mueble.IdClienteME, Mueble.IdLinea, Mueble.IdL�minaDimensi�n, Mueble.Pinta
                    FROM Mueble order by Mueble.IdMueble";

        $result = odbc_exec($conn_access, $SqlItem);
       
        while ($fila = odbc_fetch_array($result)) {
            $version = ($fila['Versi�n'] == "") ? 0 : $fila['Versi�n'];
            ...
            $Mysqlivision->query("INSERT INTO access_forniture (id_forniture,type_forniture,code_client,`version`,description,development,destination,observation,id_client,`line`,
                    sheet_format, color) VALUES (" . $fila['IdMueble'] . ", 'ME','" . $fila['C�digoCliente'] . "', " . $version . ", '" . $fila['Descripci�nMueble'] . "', '" . $fila['Descripci�nDesarrollo'] . "', '" . $fila['C�digoDestino'] . "', '" . $fila['Observaciones'] . "', " . $idcliente . ", " . $linea . ", " . $dimension . ",'" . $fila['Pinta'] . "');");
        }
	...
	case "LAMINASAX":
        ...
        while ($row = mysqli_fetch_array($result)) {
            $Sql = "update MSL�minas set idInsumoERP = '".$row[1]."' where C�digoL�minaInterno = '".$row[0]."' ";
